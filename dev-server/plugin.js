...
export function jsxDevServerPlugin() {
      ...
      server.middlewares.use((req, res, next) => {
        ...
        if (url === '/api/components') {
          try {
            ...
            res.setHeader('Content-Type', 'application/json');
            res.end(JSON.stringify({ components: [], error: 'Could not read examples directory' }));
            return;
          }
        }
        ...
        if (url.endsWith('.html') && url !== '/') {
          ...
          const jsxFile = path.join(examplesDir, `${componentName}.jsx`);
