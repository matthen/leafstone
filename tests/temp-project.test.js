...
`;
    ...
    expect(fs.existsSync(path.join(tempDir, `src/${componentName}.jsx`))).toBe(
      true
    );
...
`;
    ...
    const jsxFile = fs.readFileSync(
      path.join(tempDir, `src/${componentName}.jsx`),
      "utf-8"
    );
  ...
  test("should create main.jsx that imports the component", () => {
    ...
    const mainJsx = fs.readFileSync(
      path.join(tempDir, "src/main.jsx"),
      "utf-8"
    );
