...
function extractAssets(jsxContent, componentFile) {
  ...
  while ((match = requiresAssetRegex.exec(jsxContent)) !== null) {
    ...
    if (usedFilenames.has(filename)) {
      console.error(
        `❌ Asset filename conflict: "${filename}" is used by multiple assets`
      );
      console.error(
        `   Consider using custom filenames: // @requires-asset ${assetPath} ${filename.replace(
          ".",
          "-2."
        )}`
      );
...
module.exports = {
  ...
  if (buildMode) {
    ...
    console.log(
      `🌐 Deploy the ${outputDir} directory to any static hosting service`
    );
  } else {
    // Dev server mode
    viteConfig.server = {
    ...
    let currentDependencies = JSON.stringify(dependencies);
    let currentAssets = JSON.stringify(assets);
    ...
    fs.watchFile(componentFile, (curr, prev) => {
      if (isRestarting) return; // Prevent race condition during restart
      ...
      if (
        newDependenciesStr !== currentDependencies ||
        newAssetsStr !== currentAssets
      ) {
